...
Vagrant.configure(VAGRANTFILE_API_VERSION) do |config|
  ...
  nodes.each do |prefix, (count, ip_start)|
     count.times do |i|
	hostname = "%s#{i+1}"  % [prefix, (i+1)]

        config.vm.define "#{hostname}" do |server|
	   server.vm.hostname = "#{hostname}"
	   if prefix == "haproxy"
	   	#server.vm.provision :shell, :inline => "echo haproxy_install.sh"
	   	server.vm.provision :shell, :path => "haproxy_install.sh"
	   	server.vm.provision :shell, :path => "heartbeat_install.sh"
	   end
	   if prefix == "web"
	   	#server.vm.provision :shell, :inline => "echo web_install.sh"
